...
2. **S3 Event Trigger** – When the object lands in the bucket, a custom resource configures the bucket to invoke `ConversationImportFunction` directly on every `ObjectCreated` event. The Lambda still supports SNS-wrapped payloads, but the default path is a direct S3 → Lambda notification.
...
- `ConversationImportFunction` – Python Lambda defined in `src/import_handler.py`. Triggered by S3 `ObjectCreated:*` events configured via a custom resource.
